...
// A second, PKCE-free entry point for links minted out-of-band by
// supabase.auth.admin.generateLink() (see scripts/demo-login.mjs): the
// PKCE code_verifier that /api/auth/callback needs is a cookie the Worker
// only sets while handling a same-browser /api/auth/magic-link POST, so a
// link generated by a script -- with no such POST ever having happened in
// the visiting browser -- can never complete exchangeCodeForSession(); it
// fails silently and the browser just keeps showing whatever session
// already existed in its cookie jar. verifyOtp() has no such requirement,
// only the token_hash itself, so it works for any browser.
