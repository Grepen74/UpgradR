...
app.post("/api/auth/magic-link", async (context) => {
  ...
  // emailRedirectTo becomes {{ .RedirectTo }} in supabase/templates/magic_link.html,
  // which builds its link around {{ .TokenHash }} against /api/auth/verify below
  // rather than the default {{ .ConfirmationURL }} (which is /api/auth/callback's
  // PKCE code-exchange flow, see the comment on that route). That default is
  // unusable here: its code_verifier only ever exists as a cookie in the exact
  // browsing context that POSTed this request, but the email is very often opened
  // in a different one -- e.g. this box was installed as a desktop/home-screen web
  // app and the link is tapped from a mail client, which opens it in the plain
  // browser instead, landing the code exchange in a context lacking that cookie
  // and leaving the user looking signed-in in one place and signed-out in the
  // other. verifyOtp's token_hash carries everything it needs in the URL itself,
  // so it completes correctly no matter which browsing context opens it. A `next`
  // query param is always present (defaulting to "/") purely so the template can
  // unconditionally append `&token_hash=...&type=magiclink` after it.
  const nextPath = parsed.data.returnTo ?? "/";
  const supabase = createSupabaseServerClient(context);
  const { error } = await supabase.auth.signInWithOtp({
    email: parsed.data.email,
    options: {
      emailRedirectTo: `${context.env.APP_ORIGIN}/api/auth/verify?next=${encodeURIComponent(nextPath)}`,
    },
  });
...
// This PKCE code-exchange flow is no longer what the magic-link email (or
// scripts/demo-login.mjs's admin-generated links) points at -- both now use
// /api/auth/verify below, which works regardless of which browsing context
// opens the link (see the comment on the /api/auth/magic-link handler above).
// This route is kept only so an already-sent, not-yet-clicked email from
// before that change still completes; it's fine for it to eventually become
// unreachable dead weight once such links have all expired.
...
// The entry point every magic link now uses (see supabase/templates/magic_link.html
// and the /api/auth/magic-link handler above), plus the one used by links minted
// out-of-band by supabase.auth.admin.generateLink() (see scripts/demo-login.mjs).
// The PKCE code_verifier that /api/auth/callback above needs is a cookie the
// Worker only sets while handling a same-browser /api/auth/magic-link POST, so a
// link opened in any other browsing context -- another device, a script-generated
// link, or simply the plain browser a mail client opens links in while this app
// was requested from an installed web-app window -- can never complete
// exchangeCodeForSession(); it fails silently and the browser just keeps showing
// whatever session already existed in its cookie jar. verifyOtp() has no such
// requirement, only the token_hash itself, so it works for any browser.
app.get("/api/auth/verify", async (context) => {
  const tokenHash = context.req.query("token_hash");
  if (!tokenHash) {
    return context.redirect("/?auth_error=missing_code");
  }
