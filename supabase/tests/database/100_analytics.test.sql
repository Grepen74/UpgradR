...
-- application 2 (no shortlist) is excluded by current_status; application 1
-- has an open task so it is excluded; application 3 has neither and is
-- overdue on updates, so it is the sole next action.
select results_eq(
  $$ select application_id from public.analytics_next_actions() $$,
  $$ values ('aaaaaaaa-3333-3333-3333-333333333333'::uuid) $$,
  'analytics_next_actions() surfaces only the application with no open task'
);
